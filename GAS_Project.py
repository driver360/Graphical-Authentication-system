...
try:
    ...
    class Graph_Registration(tk.Frame):
        ...
        def update(self):
            ...
            ob = db.database()
            is_success=0
            query = "update registration_table set name = %s,email = %s,contact = %s,question =%s,answer =%s,p1 = %s,p2 = %s, p3 = %s where user_id =" + self.uid
            val = (self.R_name_var.get(), self.R_email_var.get(), self.R_contact_var.get(), self.R_question_var.get(),
                   self.R_answer_var.get(), self.R_path_list_var[0], self.R_path_list_var[1], self.R_path_list_var[2])
            is_success = ob.universal_Transact(query, val)
            if is_success == 1:
                messagebox.INFO(self,"GAS","Updated Succesfully")
            else:
                messagebox.showerror("GAS","No updated")
    ...
    class Graph_Autentication(tk.Frame):
        def __init__(self,parent,controller):
            ...
            self.j=0
            self.k=0
            self.imgs=["D:/background-1992153_1920.jpg","D:/background-378781_1920.jpg","D:/bg2.jpg","Data:/bg3.jpg","D:/black-2398956_1920.jpg","D:/background-378781_1920.jpg"]
            try:            
                #=========Register Canvas===========================
        ...
        def Forgot_Function(self):
            global a
            a=str(simpledialog.askstring("Forgot Password","Enter Your Email:-"))
            ob1 = db.database()
            query = "select question from registration_table where email = '"+a+"'"
            ob1.datalist = ob1.universal_getdata(query)
            st2=str(ob1.datalist)
            st2=st2[3:len(st2)-4]
            if(st2!=""):
                b=simpledialog.askstring("Forgot Password",st2+"?")
                ob2 = db.database()
                query1 = "select answer from registration_table where email = '"+a+"'"
                ob2.datalist = ob2.universal_getdata(query1)
                st1=str(ob2.datalist)
                st1=st1[3:len(st1)-4]
                if(b==st1):
                   self.A_controller.show_frame("Graph_Updation")
                   Graph_Updation.change(self)
                else:
                    messagebox.showerror("GAS","Answer is Incorrect!")
            else:
                messagebox.showerror("GAS","Email is Incorrect!")
    ...
    class Graph_Updation(tk.Frame):
        def __init__(self,parent,controller):
            ...
            try:            
                ...
                U_update_btn=tk.Button(self.U_Left_Canvas,text="UPDATE",command =self.update,font=("times new roman",15,"bold")).place(x=135,y=454)
        ...
        def change(self):
            ob1 = db.database()
            query = "select user_id from registration_table where email = '" + a + "'"
            ob1.datalist = ob1.universal_getdata(query)
            st2 = str(ob1.datalist)
            st2 = st2[2:len(st2) - 3]
            self.uid = int(st2)
        ...
        #=========Image Loading function===========================
        def fileDialog(self): 
            self.filename = filedialog.askopenfilename(initialdir =  "Data:/", title = "Select A File", filetype =
            (("jpeg files","*.jpg"),("all files","*.*")) )
            if(self.filename!="" and self.i<3):
